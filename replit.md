...
- **Chatbot System**:
    ...
    - **Bot Reactivation**: Agent sets custom field `reactivar_bot` to "si" in the contact's profile in Respond.io (invisible to customer). System auto-detects, reactivates bot (continues where the conversation was, not from zero), and clears the field automatically.
