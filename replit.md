...
- **Driver Commission per Stop**: User model has `commission_per_stop` (FLOAT) field. Set in driver profile edit modal in Admin Users. Displays only for role=driver. Global commission button in dispatch drivers tab allows setting commission for ALL drivers at once. Commission total per route (stops × commission_per_stop) shown in route cards and driver mobile planner.
