...
- **Chatbot System**:
    ...
    - **Bot Reactivation**: Two methods to reactivate the bot after an agent takes over: (1) Green robot button in dispatch order cards, (2) Agent sets custom field `reactivar_bot` to "si" in the contact's profile in Respond.io (invisible to customer). System auto-detects, reactivates bot, and clears the field.
