...
- **Auto-delete Delivered Orders**: Orders with status `delivered` are automatically deleted after 48 hours. Runs on server start and each scan cycle.
- **Driver Commission per Stop**: User model has `commission_per_stop` (FLOAT) field. Set in driver profile edit modal in Admin Users. Displays only for role=driver.
