        ...
        where[Op.and] = [
          {
            [Op.or]: [
              ...
              { package_disposition: { [Op.is]: null } }
            ]
          }
        ];
      }
...
router.get('/stats', requireAdmin, async (req, res) => {
  try {
    ...
    // Los registros históricos pueden no tener dispatch_status porque ese
    // campo se agregó después. NULL significa orden activa heredada, no
    // archivada, y debe aparecer tanto en el mapa como en los contadores.
    const notArchived = {
      [Op.or]: [
        { dispatch_status: { [Op.ne]: 'archived' } },
        { dispatch_status: { [Op.is]: null } }
...
router.post('/routes/:id/orders', requireAdminOrReceptionist, async (req, res) => {
  try {
    ...
    if (Array.isArray(order_ids) && order_ids.length > 0) {
      ...
      const blockedReturn = dbOrders.find(order =>
        order.package_disposition && order.package_disposition !== 'normal'
      );
      if (blockedReturn) {
        return res.status(409).json({
          error: 'El paquete debe recibirse y liberarse en la oficina antes de agregarlo a una ruta.'
        });
      }
...
router.get('/routes', requireAuth, async (req, res) => {
  try {
    ...
    const routesWithDetails = await Promise.all(routes.map(async (r) => {
      ...
      routeDict.route_total_collected = routeAllStops
        .filter(isCompletedStop)
...
router.put('/returns/:id/release', requireAdminOrReceptionist, async (req, res) => {
  try {
    ...
    if (rawId.startsWith('stop:')) {
      const stop = await Stop.findByPk(Number(rawId.slice(5)));
      if (!stop) return res.status(404).json({ error: 'Parada no encontrada' });
      if (stop.package_disposition !== 'returned_to_office') {
        return res.status(409).json({ error: 'Primero debes confirmar que el paquete fue recibido en la oficina' });
      }
    ...
    const order = await ValidatedAddress.findByPk(req.params.id);
    if (!order) return res.status(404).json({ error: 'Orden no encontrada' });
    if (order.package_disposition !== 'returned_to_office') {
      return res.status(409).json({ error: 'Primero debes confirmar que el paquete fue recibido en la oficina' });
    }
    if (order.route_id) {
...
// El flujo de doble confirmación de entrega/recogida de rutas fue retirado.
// Las columnas históricas permanecen en la base de datos, pero los endpoints
// ya no forman parte de la aplicación.
router.use('/pickup', requireAuth, (req, res) => {
  res.status(410).json({ error: 'La recepción de rutas fue eliminada' });
});
...
router.post('/pickup/:routeId/confirm-stops', requireAdminOrReceptionist, async (req, res) => {
  try {
    ...
    if (confirmedCount === 0) {
      ...
      // Notificar al DispatchMap para que refresque
      emitToAdmins('route:updated', { route_id: route.id });

      return res.json({
        success: true,
        allRejected: true,
        message: 'Todas las paradas devueltas al dispatching. Ruta desasignada.'
      });
    }
    ...
    const driver = await User.findByPk(savedDriverId, { attributes: ['id', 'username', 'email'] });
    const admin  = await User.findByPk(req.userId, { attributes: ['id', 'username', 'email'] });

    emitToDriver(savedDriverId, 'pickup:admin_confirmed', {
      route_id: route.id,
...
router.put('/routes/:id/deliver-payment', requireAuth, async (req, res) => {
  try {
    ...
    const completedStops = allStops.filter(isCompletedStop);
    const cashCollected = completedStops.reduce((sum, s) => sum + (isCashMethod(s.payment_method) ? Number(s.amount_collected) || 0 : 0), 0);
    const electronicCollected = completedStops.reduce((sum, s) => sum + (!isCashMethod(s.payment_method) ? Number(s.amount_collected) || 0 : 0), 0);
    ...
    const deliverCommission = Number(deliverDriver?.commission_per_stop || 0) * completedStops.length;
...
router.get('/my-accounting', requireAuth, async (req, res) => {
  try {
    ...
    const stopCountMap = {};
    const stopCashMap = {};
    allRouteStops.filter(isCompletedStop).forEach(s => {
...
router.get('/my-completed-routes', requireAuth, async (req, res) => {
  try {
    ...
    const result = routes.map(r => {
      ...
      const accountingStops = routeStops.filter(isCompletedStop);
      const cashCollected = accountingStops.reduce((sum, s) => sum + (isCashMethod(s.payment_method) ? Number(s.amount_collected || 0) : 0), 0);
      const electronicCollected = accountingStops.reduce((sum, s) => sum + (!isCashMethod(s.payment_method) ? Number(s.amount_collected || 0) : 0), 0);
      const totalCollected = cashCollected + electronicCollected;
      const commission = commissionPerStop * accountingStops.length;
